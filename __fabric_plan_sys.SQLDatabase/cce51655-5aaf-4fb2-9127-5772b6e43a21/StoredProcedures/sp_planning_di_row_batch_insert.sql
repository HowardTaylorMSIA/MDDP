                                  ...
                                  CREATE PROCEDURE [cce51655-5aaf-4fb2-9127-5772b6e43a21].[sp_planning_di_row_batch_insert] 
                                  ...
                                  DECLARE @TempTableName NVARCHAR(128) = '#CR_INSERT_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
                                      ...
                                      SET @sql = N'
                                          CREATE TABLE [cce51655-5aaf-4fb2-9127-5772b6e43a21].' + QUOTENAME(@TempTableName) + N' (
                                              id varchar(255),
                                              visualId int,
                                              name varchar(255),
                                    ...
                                    -- Insert data into the temporary table using dynamic SQL
                                      ...
                                      SET @sql = @sql + 'INSERT INTO [cce51655-5aaf-4fb2-9127-5772b6e43a21].data_input_row (id, name, rowMeta, visualRowConfigId, rowPath,  visualId, createdBy, updatedBy, createdAt, updatedAt)
                                      SELECT temp.id, temp.name, temp.rowMeta, temp.visualRowConfigId, temp.rowPath, temp.visualId, createdBy, updatedBy, createdAt, updatedAt FROM [cce51655-5aaf-4fb2-9127-5772b6e43a21].' + @TempTableName + ' as temp;';
