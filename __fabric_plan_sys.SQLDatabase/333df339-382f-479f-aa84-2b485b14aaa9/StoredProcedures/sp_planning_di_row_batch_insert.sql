                                  ...
                                  CREATE PROCEDURE [333df339-382f-479f-aa84-2b485b14aaa9].[sp_planning_di_row_batch_insert] 
                                  ...
                                  DECLARE @TempTableName NVARCHAR(128) = '#CR_INSERT_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
                                    ...
                                    -- Insert data into the temporary table using dynamic SQL
                                      ...
                                      SET @sql = @sql + 'INSERT INTO [333df339-382f-479f-aa84-2b485b14aaa9].data_input_row (id, name, rowMeta, visualRowConfigId, rowPath,  visualId, createdBy, updatedBy, createdAt, updatedAt)
                                      SELECT temp.id, temp.name, temp.rowMeta, temp.visualRowConfigId, temp.rowPath, temp.visualId, createdBy, updatedBy, createdAt, updatedAt FROM [333df339-382f-479f-aa84-2b485b14aaa9].' + @TempTableName + ' as temp;';
