                                  ...
                                  CREATE PROCEDURE [ce92d282-104a-43a6-8407-b1a37af182e0].[sp_planning_di_row_batch_insert] 
                                  ...
                                  DECLARE @TempTableName NVARCHAR(128) = '#CR_INSERT_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
                                    ...
                                    -- Insert data into the temporary table using dynamic SQL
                                      ...
                                      SET @sql = @sql + 'INSERT INTO [ce92d282-104a-43a6-8407-b1a37af182e0].data_input_row (id, name, rowMeta, visualRowConfigId, rowPath,  visualId, createdBy, updatedBy, createdAt, updatedAt)
                                      SELECT temp.id, temp.name, temp.rowMeta, temp.visualRowConfigId, temp.rowPath, temp.visualId, createdBy, updatedBy, createdAt, updatedAt FROM [ce92d282-104a-43a6-8407-b1a37af182e0].' + @TempTableName + ' as temp;';
