            ...
            CREATE PROCEDURE [74518bd1-854b-415e-bfdc-5e729b3db400].[sp_planning_di_row_batch_update] 
            ...
            DECLARE @TempTableName NVARCHAR(128) = '#CR_UPDATE_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
              ...
              -- Insert data into the temporary table using dynamic SQL
                ...
                SET @sql = @sql + 'UPDATE [74518bd1-854b-415e-bfdc-5e729b3db400].data_input_row 
                SET 
                    ...
                    updatedBy = @updatedBy,
                    updatedAt = @updatedAt
                FROM [74518bd1-854b-415e-bfdc-5e729b3db400].data_input_row
                INNER JOIN [74518bd1-854b-415e-bfdc-5e729b3db400].' +  @TempTableName + ' as temp on temp.id = data_input_row.id;';
