                                  ...
                                  CREATE PROCEDURE [74518bd1-854b-415e-bfdc-5e729b3db400].[sp_planning_di_row_batch_insert] 
                                  ...
                                  DECLARE @TempTableName NVARCHAR(128) = '#CR_INSERT_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
                                      ...
                                      SET @sql = N'
                                          CREATE TABLE [74518bd1-854b-415e-bfdc-5e729b3db400].' + QUOTENAME(@TempTableName) + N' (
                                              id varchar(255),
                                              visualId int,
                                              name varchar(255),
                                    ...
                                    -- Insert data into the temporary table using dynamic SQL
                                      ...
                                      SET @sql = @sql + 'INSERT INTO [74518bd1-854b-415e-bfdc-5e729b3db400].data_input_row (id, name, rowMeta, visualRowConfigId, rowPath,  visualId, createdBy, updatedBy, createdAt, updatedAt)
                                      SELECT temp.id, temp.name, temp.rowMeta, temp.visualRowConfigId, temp.rowPath, temp.visualId, createdBy, updatedBy, createdAt, updatedAt FROM [74518bd1-854b-415e-bfdc-5e729b3db400].' + @TempTableName + ' as temp;';
